...
Parent epic: `lspm-y5n`, Phase 2. Depends on Phase 1 (`lspm-cnq`).
...
- [ ] `authoring-lsp-plugin` SKILL.md ships with schema reference, concrete manifest examples, buildHook patterns (clone-and-build, install-from-registry, prebuilt-binary-fetch), capability flag semantics, and passes `skill-reviewer` agent review.
...
- [ ] `validate-manifest` utility: CLI invocation `bun run validate-manifest <path>` produces human-readable pass/fail output; used in CI to validate every shipped manifest.
- [ ] `bun run test` passes: existing Phase 1 tests + new Phase 2 tests covering settings parsing, settings application (disable / override / primary), fork buildHook idempotency, validate-manifest against valid + invalid fixtures.
...
- **NO CI smoke test that mocks the fork binary.** Test must exercise actual buildHook output against real LSP startup. Otherwise regressions from fork-upstream drift go undetected.
...
- **Fork upstream drift**: Each fork tracks a moving target (`sethyanow/pyright` dev branch, `sethyanow/zls` dev branch, `sethyanow/markymark` main). buildHook must pin to a specific commit or tag, not a branch head, to prevent silent breakage. Bumping the pin is a deliberate act.
- **buildHook timeout budget**: Phase 1 commit `476a855` addressed buildHook stdio + warm-up timeout budget. Phase 2 fork buildHooks must respect it — clones + installs + builds may bump against it. First-run-takes-N-minutes is user-observable; subsequent runs must be fast via idempotency.
...
- **Per-lang primary conflict**: Settings `primary:` map overrides first-registered default. If a user pins a manifest that's `binary_not_found`: log an error; fall back to first-registered among `ok`-status candidates; user sees the fallback in `list_languages`.
...
- **`.local.md` vs `LSP_MCP_SETTINGS` priority**: First source wins. If a user sets both, explicit env wins over convention — ergonomic for CI overrides.
...
- [ ] Any Phase 1 docs that reference "fork support coming in Phase 2" get updated to point at the shipped fork plugins.
