...
- [2026-04-20T05:52:24Z] [Seth] CLOSED (commits 1054ba0 + 8aca216). Debrief: No workarounds. One design correction: skeleton's step 3 predicate (slot.candidates.some) was structurally incorrect — _buildLangMap filters binary_not_found from slot.candidates, making step 4 (the binary_not_found error) unreachable. Fix: step 3 uses entry.manifest.langIds.includes(lang) — author-declared view matches mutation-path intent. Extracted Router._binaryNotFoundError(name, status) static helper so _requireByName + setPrimary share phrasing (triggered by user calling out my 'rule of three' extraction-threshold dodge — feedback memory saved). Next task inherits: R7b can derive enum values from same manifest set; acceptance demo uses set_primary for bazel starpls↔bazel-lsp swap showcase. Reflections: SRE gap (I noted _buildLangMap filter but didn't trace its interaction with setPrimary validation chain — test failure caught it). User corrections: (1) no extraction threshold especially for test helpers; (2) don't AskUserQuestion after already naming best option — that's decision-laundering. Both in feedback_no_extraction_threshold.md. Skeleton was otherwise accurate (fixture conventions, MCP patterns, return shape, anti-patterns, SC count target 220-223 → actual 223). Memory cycle: 1 new feedback memory (no_extraction_threshold), 0 stale edits needed. Cross-pollination: _binaryNotFoundError extraction could be used by future Router methods — not urgent.
