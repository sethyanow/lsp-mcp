...
type: epic
priority: 1
depends_on: [lspm-501, lspm-z4z, lspm-177, lspm-h1n, lspm-kgj, lspm-mcp, lspm-hlm, lspm-rot, lspm-zw9, lspm-4vb, lspm-8cu, lspm-xxk]
parent: lspm-y5n
---
...
- [x] `set_primary(lang, manifest)` MCP tool swaps primary in-memory without restart. *(satisfied by `lspm-zw9` — `Router.setPrimary(lang, manifestName): {lang, primary, previous}` in src/router.ts; `set_primary` MCP tool in src/mcp-server.ts registered adjacent to `list_languages`; 4-step fail-fast validation (unknown manifest → unknown lang → not-a-candidate-by-manifest.langIds → binary_not_found); idempotent no-op when target === current; mutation-before-log write-order invariant; stderr `[lsp-mcp] set_primary: <lang> <prev> → <new>`; shared `Router._binaryNotFoundError` static helper deduped the error phrasing with `_requireByName`. 21 new tests: 17 router unit (happy-path, listLanguages-reflects, 2 no-ops, 4 validation throws, stderr log, 8-pattern adversarial battery + 3 adversarial-stress-test additions) + 4 MCP integration (registration, swap+list_languages round-trip, JSON round-trip, 4-case error batch). 223 green across 7 suites. Smoke: single-candidate no-op, unknown-manifest + unknown-lang errors all shape-verified with informative messages listing known alternatives. R7b dynamic schemas, R9 using-lsp-mcp skill, acceptance demo still open.)*
...
- [x] MCP tool input schemas built dynamically at startup; `lang` / `langs` / `via` / `manifests` parameters expose enum values reflecting currently-active manifests. *(satisfied by `lspm-4vb` — `buildDynamicSchemas(router)` factory in src/mcp-server.ts (~30 LOC) emits 5 schemas (LangEnum, LangsSchema, ManifestEnum, ManifestsSchema, ViaSchema) at createMcpServer time; wired into symbol_search.langs/manifests, set_primary.lang/manifest, lsp.lang, and via on all 10 positional tools (defs/refs/impls/hover/outline/diagnostics/lsp/call_hierarchy_prepare/incoming_calls/outgoing_calls). enumOrString helper handles empty-list fallback to plain z.string() (zod's z.enum([]) throws). Module-scope ViaSchema const removed; both R7 TODO comments deleted. Schemas STABLE across set_primary swaps (anti-pattern lock test). 17 new tests (+1 cycle conversion = 18 actual; net +17): cycle-1 symbol_search.langs enum, cycle-2 symbol_search.manifests + binary_not_found EXCLUDED, cycle-3 positional via enum (10 tools), cycle-4 set_primary.lang+manifest required + lsp.lang single-LangEnum-source deep-equal, cycle-4b 1 surfaces test split into 3 (cross-dispatch survives MCP, router-direct triple, new MCP Zod-rejection negative coverage), cycle-5 empty-router fallback per-param required-vs-optional, cycle-6 all-binary_not_found fallback, cycle-7 set_primary swap stability, adversarial battery (singular, dense-20, multi-langId, redundant-langIds, 10-swap stability, second-run idempotency, encoding-boundaries non-ASCII). Smoke harness extended with --inspect-schema flag; observed enums on dev box: 13 active langs, 7 ok manifests, 5 binary_not_found EXCLUDED. Sub-task 8b discovered + closed: 3 of 4 R7 negative-test cases needed migration to router-direct (Zod intercepts before router error firing). Test baseline: 226 → 243. Acceptance demo + R9 using-lsp-mcp skill remain.)*
...
- [x] `skills/using-lsp-mcp/SKILL.md` (at repo root) ships with third-person description, specific trigger phrases for polyglot / symbol lookup / cross-language refactor queries, imperative body, concrete examples for Python↔Rust (pyo3), TS↔Go (gRPC), and C embedded in anything. No position-counting from text in examples. *(satisfied by `lspm-8cu` — 1822-word skill body, 7 sections (activation → discovery pattern → 3 cross-language examples → tool surface → routing dials → anti-patterns → failure modes); third-person 368-char description leading with "This skill should be used when..."; three cross-language examples each chain `symbol_search → defs/refs/hover` with `// ← from <lsp>-hit.range.start` provenance annotations on every numeric `pos` (zero fabricated positions); tool-surface section split correctly — `hover {file, pos, via?}` with `pos` required, `outline`/`diagnostics {file, via?}` with no `pos`, `call_hierarchy_prepare {file, pos, via?}` vs `incoming_calls`/`outgoing_calls {item, via?}` — this last split avoids the "invalid MCP call" mistake mode; 6 anti-patterns locked (position-counting, `list_languages` pre-flight, file-iteration, unknown-language `langs` filter, mid-session `set_primary`, invented enum values); "example from a dev box" disclaimer on inline enum values per Stale Enum Values mitigation. skill-reviewer (plugin-dev:skill-reviewer) passed at iteration 3 of 3 — iteration 1 flagged 2 critical + 2 major + 7 minor (all applied with source-of-truth verification against src/mcp-server.ts); iteration 2 flagged 1 major + 3 minor (1 major applied, 2 minors rejected with documented reasoning: "three dials" framing is intentionally scoped to post-discovery routing, and stable manifest names are distinct from runtime-enum langIds); iteration 3 clean pass with zero findings. Regression: 243 tests green, typecheck clean, bun run build unaffected. Acceptance task (README + cold CC session demo) remains open.)*
...
- [x] `skills/using-lsp-mcp/SKILL.md` ships as new content (R9); not a stale-doc update but a Phase 1 deliverable. *(satisfied by `lspm-8cu` — see main SC above for details)*
